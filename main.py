...
def user_home():
    ...
    def ins():
        ...
        if(all([select_city.get(),select_locations.get(),select_shops.get(),phoneno.get(),rcnum.get(),service.get()])):
            ...
            if (list(db)[0][0] >= 5):
                ...
                sql = "INSERT INTO bookings VALUES (%s,%s,%s,%s,%s,%s)"
                db.execute(sql,[select_city.get(),select_locations.get(),select_shops.get(),phoneno.get(),rcnum.get(),service.get()])
                mydb.commit()
    ...
    Label(ui, text='Car Wash Booking', font=('Arial', 30)).place(x=400, y=50)
    signup_x = 300
    signup_y = 270
...
def admin_home():
    ...
    Button(ah, text="Add Services", font=('Arial 10'), command=AddServices, width=10, height=1).place(x=600, y=20)
    ...
    try:
        ...
        if (str(username_s.get()) in [i[0] for i in db]):
            ...
            if re.search('[A-Z]', val[1]) and re.search('[a-z]', val[1]) and re.search('[0-9]', val[1]) and re.search(
                ...
                db.execute(sql, val)
                tb.insert(INSERT, "Inserted Successfully\n")
            else:
...
Button(ws, text="SignUp", font=('Arial 10'), command=registration, width=10, height=2).place(x=signup_x + 200,
                                                                                             y=signup_y + 100)
